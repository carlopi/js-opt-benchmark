...
var a0: number, a1: number, a2: number, a3: number, b1: number, b2: number, left_corner: number, right_corner: number;
...
function convolveRGBA(src: Uint32Array, out: Uint32Array, line: Float32Array, coeff: Float32Array, width: number, height: number): void {
  // takes src image and writes the blurred and transposed result into out

  var rgba: number;
  var prev_src_r: number, prev_src_g: number, prev_src_b: number, prev_src_a :number;
  var curr_src_r:number, curr_src_g:number, curr_src_b:number, curr_src_a:number;
  var curr_out_r:number, curr_out_g:number, curr_out_b:number, curr_out_a:number;
  var prev_out_r:number, prev_out_g:number, prev_out_b:number, prev_out_a:number;
  var prev_prev_out_r:number, prev_prev_out_g:number, prev_prev_out_b:number, prev_prev_out_a:number;

  var src_index: number, out_index:number, line_index:number;
  var i:number, j:number;
  var coeff_a0:number, coeff_a1:number, coeff_b1:number, coeff_b2:number;
...
export function blurRGBA(src: Uint8ClampedArray, width: number, height: number, radius: number): Uint8ClampedArray {
